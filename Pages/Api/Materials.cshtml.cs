...
namespace WiseLabels.Pages.Api
{
    ...
    public class MaterialsModel : PageModel
    {
        ...
        public MaterialsModel(IConfiguration configuration, IHttpClientFactory httpClientFactory, ILogger<MaterialsModel> logger)
        {
        ...
        private async Task<object> FetchMaterialsAsync(string materialsUrl, string accessToken)
        {
            try
            {
                ...
                var jsonResponse = await response.Content.ReadAsStringAsync();
                _logger.LogDebug("Materials API response: {Response}", jsonResponse);
                ...
                try
                {
                    ...
                    {
                        paramResponse = JsonSerializer.Deserialize<List<ParameterResponse>>(jsonResponse);
                    }
                    // Check if it's wrapped in a Data property
                    else if (jsonDoc.TryGetProperty("Data", out var data) && data.ValueKind == JsonValueKind.Array)
                    {
                        paramResponse = JsonSerializer.Deserialize<List<ParameterResponse>>(data.GetRawText());
                    }
                    ...
                    {
                        paramResponse = JsonSerializer.Deserialize<List<ParameterResponse>>(items.GetRawText());
                    }
                    else if (jsonDoc.TryGetProperty("results", out var results) && results.ValueKind == JsonValueKind.Array)
                    {
                        paramResponse = JsonSerializer.Deserialize<List<ParameterResponse>>(results.GetRawText());
                    }
                    else
                    {
                        _logger.LogWarning("Unexpected JSON structure. Root kind: {ValueKind}", jsonDoc.ValueKind);
                        // Try direct deserialization anyway
                        paramResponse = JsonSerializer.Deserialize<List<ParameterResponse>>(jsonResponse);
                    }
                }
                ...
                if(paramResponse != null && paramResponse.Count > 0)
                {
                    ...
                    {
                        if(param.Descriptions != null && param.Descriptions.Count > 0)
                        {
                            foreach (var desc in param.Descriptions)
                            {
                                // Log each description for debugging
                                _logger.LogDebug("Material ID: {MaterialID}, Language: {Language}, Description: {Description}",
                                    param.ID, desc.ISOLanguageCode, desc.Description);
                            }
                        }
                        else
                        {
